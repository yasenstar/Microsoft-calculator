...
/*
 * Ready function, which does the following,
...
$(document).ready(function() {
    ...
    // Add keypress event to main textbox to prevent the alphabets and allow only numbers to enter
    $('#mainDisplay').keypress(function( event ) {
...
function pushToMemory(value) {
    if(isNumber(value)) {
        pushNumberToStack(value);
    } else {
        pushOperatorToStack(value);
    }
    memory += value;
...
/*
 * Function to push number value to stack, we need this method to replace the last array value
 * with the given number, if the last value is number. If the last value is operator, push the given value to stack
 *
 * This will make sure that the stak will have the numbers and operators in an alternate sequence
 */
function pushNumberToStack(value) {
  if(isNumber(stack[stack.length - 1])) {
    replaceStackLastValue(value);
  }
  else {
    pushToStack(value);
  }
...
/*
 * Function to push operator to stack, we need this method to push the entered operator value to stack
 * if the last value of the stack is again an operator, replace the last value with the entered operator
 *
 * This will make sure that the stak will have the numbers and operators in an alternate sequence
 */
function pushOperatorToStack(value) {
...
/*
 *  Function to push the given value to stack 
 */
...
/*
 * Function to replace the stack array last value with the given value 
 */
function replaceStackLastValue(value) {
  stack.pop();
  stack.push(value);
  console.log(stack);
}
...
function calculate(leftOperand, operator, rightOperand) {
    if(!isEqualPressed && !infinite) {
        ...
        isInputComplete(isNumberAlreadyPressed) ? mainDisplay(currencyFormat(leftOperand)) : mainDisplay(leftOperand); 
        isNumberAlreadyPressed = false;
        isOperatorPressed = false;
    }
...
function checkOutput(value) {
    ...
    if (value === Infinity || value === -Infinity) {
        mainDisplay('Cannot divide by 0');
        infinite = true;
        return true;
    } else if(isNaN(value) === true) {
        // if 0 is divided by 0, this code executes
        mainDisplay('Result is undefined');
        infinite = true;
        return true;
    }
    return false;
...
function removeValue(value) {
    ...
    if(value === '') {
        ...
        mainDisplay(currencyFormat(value));
    }
    digitCount--;
    return value;
}
